...
class Route
{
    ...
    protected function compileRoute()
    {
        ...
        $this->compiled = (new SymfonyRoute($uri, $optionals, $this->wheres, [], $this->domain() ?: ''))->compile();
    }
    ...
    protected function makeInvokableAction($action)
    {
        if (!method_exists($action, '__invoke')) {
            throw new UnexpectedValueException(sprintf('Invalid route action: [%s]', $action));
        }
    ...
    public static function getValidators()
    {
        ...
        return static::$validators = [new UriValidator(), new MethodValidator(), new SchemeValidator(), new HostValidator()];
    }
    ...
    public function uses($action)
    {
        ...
        return $this->setAction(array_merge($this->action, $this->parseAction(['uses' => $action, 'controller' => $action])));
    }
...
class Factory implements FactoryContract
{
    ...
    public function addLoop($data)
    {
        ...
        $this->loopsStack[] = ['iteration' => 0, 'index' => 0, 'remaining' => isset($length) ? $length : null, 'count' => $length, 'first' => true, 'last' => isset($length) ? $length == 1 : null, 'depth' => count($this->loopsStack) + 1, 'parent' => $parent ? (object) $parent : null];
    }
